...
namespace ItechPanel\Configurator\Block\Adminhtml\Section\Tab;
...
class Product extends Extended
{
    ...
    {
        parent::_construct();
        $this->setId('section_product_grid');
    ...
    {
        return $this->coreRegistry->registry('current_section');
    }
    ...
    {
        $products = $this->getRequest()->getPost('section_products');
        
        if ($products === null) {
            // First: Look for AJAX cached state
            if (!empty($this->selectedProducts)) {
                return array_keys($this->selectedProducts);
            }
            
            // Second: Pull directly from Database on initial page load
            ...
            if ($section && $section->getId()) {
                ...
                $select = $connection->select()->from($tableName, ['product_id'])->where('section_id = ?', $section->getId());
    ...
    // This makes the "Yes/No/Any" dropdown on the checkbox column work natively
    ...
    {
        return $this->getUrl('configurator/section/productsgrid', ['_current' => true]);
    }
}