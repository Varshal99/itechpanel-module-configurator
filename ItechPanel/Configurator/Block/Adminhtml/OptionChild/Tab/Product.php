...
namespace ItechPanel\Configurator\Block\Adminhtml\OptionChild\Tab;
...
class Product extends Extended
{
    ...
    protected function _construct() {
        parent::_construct();
        $this->setId('optionchild_product_grid');
        $this->setDefaultSort('entity_id');
        $this->setUseAjax(true);
    }

    public function getOptionChild() { return $this->coreRegistry->registry('current_optionchild'); }
    public function setSelectedProducts($products) { $this->selectedProducts = $products; return $this; }

    public function getSelectedProducts() {
        $products = $this->getRequest()->getPost('optionchild_products');
        if ($products === null) {
            if (!empty($this->selectedProducts)) return array_keys($this->selectedProducts);
            $entity = $this->getOptionChild();
            if ($entity && $entity->getId()) {
                $conn = $this->resourceConnection->getConnection();
                $table = $this->resourceConnection->getTableName('itechpanel_configurator_option_child_product');
                return $conn->fetchCol($conn->select()->from($table, ['product_id'])->where('child_id = ?', $entity->getId()));
            }
            return [];
        }
    ...
    public function getGridUrl() { return $this->getUrl('configurator/optionchild/productsgrid', ['_current' => true]); }
}