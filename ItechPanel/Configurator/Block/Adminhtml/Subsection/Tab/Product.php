...
namespace ItechPanel\Configurator\Block\Adminhtml\Subsection\Tab;
...
class Product extends Extended
{
    ...
    {
        parent::_construct();
        $this->setId('subsection_product_grid');
    ...
    {
        return $this->coreRegistry->registry('current_subsection');
    }
    ...
    {
        $products = $this->getRequest()->getPost('subsection_products');
        ...
        if ($products === null) {
            ...
            $entity = $this->getSubsection();
            if ($entity && $entity->getId()) {
                $connection = $this->resourceConnection->getConnection();
                $tableName = $this->resourceConnection->getTableName('itechpanel_configurator_subsection_product');
                $select = $connection->select()->from($tableName, ['product_id'])->where('subsection_id = ?', $entity->getId());
    ...
    {
        return $this->getUrl('configurator/subsection/productsgrid', ['_current' => true]);
    }
}