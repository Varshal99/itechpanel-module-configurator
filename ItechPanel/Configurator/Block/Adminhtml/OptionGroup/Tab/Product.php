...
namespace ItechPanel\Configurator\Block\Adminhtml\OptionGroup\Tab;
...
class Product extends Extended
{
    ...
    protected function _construct() {
        parent::_construct();
        $this->setId('optiongroup_product_grid');
        $this->setDefaultSort('entity_id');
        $this->setUseAjax(true);
    }

    public function getOptionGroup() { return $this->coreRegistry->registry('current_optiongroup'); }
    public function setSelectedProducts($products) { $this->selectedProducts = $products; return $this; }

    public function getSelectedProducts() {
        $products = $this->getRequest()->getPost('optiongroup_products');
        if ($products === null) {
            if (!empty($this->selectedProducts)) return array_keys($this->selectedProducts);
            $entity = $this->getOptionGroup();
            if ($entity && $entity->getId()) {
                $conn = $this->resourceConnection->getConnection();
                $table = $this->resourceConnection->getTableName('itechpanel_configurator_option_group_product');
                return $conn->fetchCol($conn->select()->from($table, ['product_id'])->where('group_id = ?', $entity->getId()));
            }
            return [];
        }
    ...
    public function getGridUrl() { return $this->getUrl('configurator/optiongroup/productsgrid', ['_current' => true]); }
}